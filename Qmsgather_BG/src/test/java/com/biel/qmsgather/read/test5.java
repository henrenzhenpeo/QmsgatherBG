...
//
// import com.biel.qmsgather.domain.DfYfExcelYf115C98bBlueSandblastingGridTestS3Data;
// import com.biel.qmsgather.mapper.DfYfExcelYf115C98bBlueSandblastingGridTestS3DataMapper;
...
//
//     @Autowired
//     private DfYfExcelYf115C98bBlueSandblastingGridTestS3DataMapper mapper;
//
//     @Test
...
//         String sheetName = "MEK";
//         int startRow = 4;
//         int readColumns = 5; // 指定要读取的列数
//
//         List<DfYfExcelYf115C98bBlueSandblastingGridTestS3Data> dataList = new ArrayList<>();
//         int successCount = 0;
//         int failCount = 0;
//
...
//
//                 try {
//                     DfYfExcelYf115C98bBlueSandblastingGridTestS3Data data = new DfYfExcelYf115C98bBlueSandblastingGridTestS3Data();
//
//                     // 设置实体类字段，按顺序一对一对应
//                     data.setTest2d(getMergedCellValue(sheet, i, 0, row.getCell(0), evaluator, formatter));
//                     data.setTestNo(getMergedCellValue(sheet, i, 1, row.getCell(1), evaluator, formatter));
//                     data.setTestTime(getMergedCellValue(sheet, i, 2, row.getCell(2), evaluator, formatter));
//                     data.setTestResult(getMergedCellValue(sheet, i, 3, row.getCell(3), evaluator, formatter));
//
//                     // 设置创建时间和更新时间
//                     Date now = new Date();
...
//             // 批量保存数据到数据库
//             if (!dataList.isEmpty()) {
//                 for (DfYfExcelYf115C98bBlueSandblastingGridTestS3Data data : dataList) {
//                     try {
//                         mapper.insert(data);
//                         successCount++;
//                     } catch (Exception e) {
...
//                     }
//                 }
//                 log.info("成功保存{}条数据到数据库", successCount);
//             } else {
//                 log.info("没有有效数据需要保存");
//             }
//
...
//         }
//
//         log.info("Excel文件读取完成，成功导入: {}条，失败: {}条", successCount, failCount);
//     }
//
//     /**
//      * 检查数据是否有效（至少有一个非null或非"null"字段）
//      */
//     private boolean isValidData(DfYfExcelYf115C98bBlueSandblastingGridTestS3Data data) {
//         return (isValidField(data.getTest2d()) ||
//                 isValidField(data.getTestNo()) ||
